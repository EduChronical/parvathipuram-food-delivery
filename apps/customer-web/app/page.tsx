...
export default function CustomerApp(){
 ...
 if(selected)return <div className="min-h-screen bg-[#f8f5ef]">
   ...
   <main className="mx-auto max-w-6xl p-4 md:p-7">
     ...
     {(selected.reviews?.length??0)>0&&<section className="card mb-24"><div className="split"><div><span className="eyebrow">VERIFIED REVIEWS</span><h2 className="text-xl font-bold">What customers say</h2></div><Pill>{selected.reviews?.length??0} recent</Pill></div><div className="grid cols-2 mt-4">{(selected.reviews??[]).map((r:any)=><div key={r.id} className="rounded-xl border border-[#e9e4dc] p-3"><b>★ {r.restaurantRating}/5</b><p className="mt-2 text-sm">{r.text||"No written comment"}</p>{r.restaurantReply&&<p className="muted mt-2 text-sm"><b>Restaurant reply:</b> {r.restaurantReply}</p>}</div>)}</div></section>}
   </main>
