...
export async function authRoutes(app:FastifyInstance){
  app.post("/auth/register",{config:{rateLimit:{max:5,timeWindow:"10 minutes"}}},async(req,reply)=>{
    ...
    try{
      ...
      req.log.error({err,userId:user.id},"Registration email delivery failed; allowing password session with deferred verification");
      return {...await issueTokens(app,user.id,{ip:req.ip,ua:req.headers["user-agent"]}),verificationRequired:false,verificationDeferred:true};
    }
  });
  ...
  app.post("/auth/otp/verify",{config:{rateLimit:{max:10,timeWindow:"10 minutes"}}},async(req,reply)=>{
    ...
    if(!user){
      const role=await db.role.findUniqueOrThrow({where:{code:RoleCode.CUSTOMER}});
      user=await db.user.create({data:{...(isEmail?{email:body.destination.toLowerCase(),emailVerifiedAt:new Date()}:{phone:body.destination,phoneVerifiedAt:new Date()}),profile:{create:{name:body.name??"Customer"}},wallet:{create:{}},roles:{create:{roleId:role.id}}}});
  ...
  app.post("/auth/refresh",async(req,reply)=>{
    ...
    if(!session||session.revokedAt||session.expiresAt<=new Date()||!(await argon2.verify(session.refreshTokenHash,parsed.secret))) return reply.code(401).send({code:"INVALID_REFRESH_TOKEN",message:"Refresh token invalid",requestId:req.id});
  ...
  app.post("/auth/logout",async(req)=>{const body=z.object({refreshToken:z.string().min(20)}).parse(req.body);const parsed=splitRefreshToken(body.refreshToken);if(parsed) await db.session.updateMany({where:{id:parsed.sessionId,revokedAt:null},data:{revokedAt:new Date()}});return {ok:true}});
  app.post("/auth/logout-all",async(req)=>{const u=await requireAuth(req);await db.session.updateMany({where:{userId:u.id,revokedAt:null},data:{revokedAt:new Date()}});return {ok:true}});
  app.post("/auth/password",async(req)=>{const u=await requireAuth(req);const body=z.object({currentPassword:z.string().min(8),newPassword:z.string().min(10)}).parse(req.body);const user=await db.user.findUniqueOrThrow({where:{id:u.id}});if(!user.passwordHash || !(await argon2.verify(user.passwordHash,body.currentPassword))) throw Object.assign(new Error("Current password invalid"),{statusCode:400,code:"INVALID_PASSWORD"});await db.user.update({where:{id:u.id},data:{passwordHash:await argon2.hash(body.newPassword)}});await db.session.updateMany({where:{userId:u.id,revokedAt:null},data:{revokedAt:new Date()}});return {ok:true}});
}
